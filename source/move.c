...
void moveright(ROBOT_CASE *robot_position)//范围：x-20~x+35,y-80~y-3
{
	int i,x0,y0,count=0,hand_count=0;//count用于计步，改变双脚移动情况,hand_count手部动作
	(*robot_position).leg_left=0;
	(*robot_position).leg_right=0;
	(*robot_position).leg=1;
	(*robot_position).cat = (unsigned int*)malloc(55*77*sizeof(unsigned int));
	(*robot_position).hand_right=0;
	(*robot_position).hand=0;
	get_image((*robot_position).x-20,(*robot_position).y-80,(*robot_position).x+35,(*robot_position).y-3,(*robot_position).cat);
	for(i=0;i<200;i++)
	{
		if(count>=5)
		{
		...
		delay0(150);
		put_image((*robot_position).x-20,(*robot_position).y-80,(*robot_position).x+35,(*robot_position).y-3,(*robot_position).cat);
		x0 = (*robot_position).x+3;
		
		get_image(x0-20,(*robot_position).y-80,x0+35,(*robot_position).y-3,(*robot_position).cat);
        if((*robot_position).leg)
		{
			(*robot_position).leg_right=0;
			(*robot_position).leg_left--;
